...
def VerifySubAdditivity(inpchans, refchans, method="chflow", chan_from="chflow"):
	...
	return (dist_sum, composite_dnorm, difference)
...
def GetChansFromChflow(chname, nqubits, *params):
	...
	if chname == "rtasu":
		...
		rand_kraus = [GetKraussForChannel(chname, *params) for __ in range(nqubits)]
...
def PlotSubAdd(nqubits, nrates, nsamples):
	"""
	Plot the subaddivity data. For channel ensembles, plot the
	"""
	subadd_data = np.load(
		"./../../temp/subadd_n_%d_r_%d_s_%d.npy" % (nqubits, nrates, nsamples)
	)
	# print("subadd_data\n{}".format(subadd_data))
	plotfname = "./../../temp/subadd_r_%d_s_%d.pdf" % (nrates, nsamples)
	with PdfPages(plotfname) as pdf:
		fig = plt.figure(figsize=(40, 33))
		# Plot
		xaxis = subadd_data[:, :, 2].ravel()
		yaxis = subadd_data[:, :, 3].ravel()
		plt.plot(xaxis, xaxis, linestyle="-.", label="X=Y")
		plt.plot(
			xaxis,
			yaxis,
			color="r",
		...
		ax = plt.gca()
		ax.set_xlabel(
			"$\\sum_{i=1}^{%d}||\\mathcal{E}_{i} - id||_{\\diamondsuit}$" % (nqubits),
			fontsize=gv.axes_labels_fontsize,
		)
		ax.set_ylabel(
			"$||\\mathcal{E}_{1}\\otimes\\ldots\\otimes\\mathcal{E}_{%d} - id||_{\\diamondsuit}$" % (nqubits),
			fontsize=1.2*gv.axes_labels_fontsize
		)
		ax.tick_params(
			axis="both",
			which="both",
			pad=gv.ticks_pad*1.5,
			direction="inout",
		...
		ax.legend(
			...
			loc=4,
			shadow=True,
			fontsize=gv.legend_fontsize * 1.25,
		...
		pdfInfo[
			"Title"
		] = "Verifying the sub-additivity property of the diamond distance."
...
if __name__ == "__main__":
	...
	nsamples = 100
	nqubits = 3
	subaddfile = "./../../temp/subadd_n_%d_r_%d_s_%d.npy" % (nqubits, nrates, nsamples)
	if os.path.isfile(subaddfile):
		print("Plotting existing data.")
		PlotSubAdd(nqubits, nrates, nsamples)
		sys.exit(0)

	subadd = np.zeros((nrates, nsamples, 5), dtype=np.double)
	for r in range(nrates):
		np.random.seed()
		print("Rate: {}".format(r + 1))
		rate = (r+1) * 0.01
		for s in range(nsamples):
			print("Sample: {}".format(s + 1))
			subadd[r, s, :2] = (rate, s)
			(rand_chans, iden_chans) = GetChansFromChflow("rand", nqubits, rate)
			if test_mode == "compare":
				CompareDNormMethods(rand_chans, iden_chans, chan_from="qutip")
			elif test_mode == "subadd":
				subadd[r, s, 2:] = VerifySubAdditivity(
					rand_chans, iden_chans, method="chflow", chan_from="chflow"
				)
			else:
				pass
			print("xxxxxxxxxxx")
	np.save(subaddfile, subadd)
	with open(chanlog, "a") as bc:
		bc.write("%%%%%%%%%%%%%% End of log.")
