...
def ThresholdPlot(phymets, logmet, dbs):
	...
	with PdfPages(plotfname) as pdf:
		for m in range(len(phylist)):
			...
			ax.set_ylabel("$\\widetilde{\\mathcal{N}}_{\\ell}: %s$" % (ml.Metrics[logmet][1].replace("$","")), fontsize = gv.axes_labels_fontsize)
			# ax.set_yscale('log')
	...
	with PdfPages(plotfname) as pdf:
		for l in range(1 + maxlevel):
			...
			for p in range(len(phylist)):
				for d in range(ndb):
					logErrs = np.load(fn.LogicalErrorRates(dbses[d], logmet, fmt = "npy"))[:, l]
					...
					if (not (dbses[d].timestamp in [name[0] for name in dbnames])):
						...
						if (l == 0):
							dbnames.append([dbses[d].timestamp, (("N = 1, D = 1, %s") % (qc.Channels[dbses[d].channel][0]))])
						dbnames.append([dbses[d].timestamp, (("N = %d, D = %d, %s") % (np.prod([dbses[d].eccs[j].N for j in range(l)]), np.prod([dbses[d].eccs[j].D for j in range(l)]), qc.Channels[dbses[d].channel][0]))])
...
def LevelWisePlot2D(phymets, logmet, dbs):
	...
	with PdfPages(plotfname) as pdf:
		...
		#Set PDF attributes
		pdfInfo = pdf.infodict()
		pdfInfo['Title'] = ("%s vs. %s at levels %s, for %d %s channels." % (ml.Metrics[logmet][0], ",".join(phylist), ", ".join(list(map(lambda str: "%d" % str, range(1, 1 + dbs.levels)))), dbs.channels, qc.Channels[dbs.channel][0]))
		pdfInfo['Author'] = "Pavithran Iyer"
		pdfInfo['ModDate'] = dt.datetime.today()
	return None
...
def CompareAnsatzToMetrics(dbs, pmet, lmet):
	...
	with PdfPages(plotfname) as pdf:
		for l in range(1 + dbs.levels):
			...
			fitlines[0, :] = cl.ComputeBestFitLine(np.column_stack((np.log10(phyerr), logerr[:, l])))
			fitlines[1, :] = cl.ComputeBestFitLine(np.column_stack((np.log10(fiterr), logerr[:, l])))
			...
			plt.plot(np.power(fiterr, fitlines[1, 0]/fitlines[0, 0]), logerr[:, l], label = ("$\\epsilon$ where $\\widetilde{\\mathcal{N}}_{%d} = %s \\times \\left[\\epsilon(\\mathcal{E})\\right]^{%.2f t}$" % (l, latex_float(weightenums[l]), expo)), color = 'blue', marker = 'o', markersize = gv.marker_size, linestyle = 'None')
			# Axes
			ax = plt.gca()
			ax.set_xlabel("$\\mathcal{N}_{0}$", fontsize = gv.axes_labels_fontsize)
			ax.set_xscale('log')
			ax.set_ylabel(("$\\mathcal{N}_{%d}$  $\\left(%s\\right)$" % (l, ml.Metrics[lmet][1].replace("$", ""))), fontsize = gv.axes_labels_fontsize)
			ax.set_yscale('log')
			ax.tick_params(axis = 'both', which = 'both', pad = gv.ticks_pad, direction = 'inout', length = gv.ticks_length, width = gv.ticks_width, labelsize = gv.ticks_fontsize)
			# Legend
			...
			bins = ComputeBinVariance(np.power(np.abs(fiterr), fitlines[1, 0]/fitlines[0, 0]), -np.log10(np.abs(logerr[:, l])))
			AddBinVariancePlot(bins, l, lmet, 0, "$\\epsilon(\\mathcal{E})$", pdf)
		#Set PDF attributes
		pdfInfo = pdf.infodict()
		pdfInfo['Title'] = ("Comparing fit obtained p to physical %s at levels %s, by studying fluctuations of output %s for %d channels." % (ml.Metrics[pmet][0], ", ".join(list(map(lambda str: "%d" % str, range(1, 1 + dbs.levels)))), ml.Metrics[lmet][0], dbs.channels))
		pdfInfo['Author'] = "Pavithran Iyer"
		pdfInfo['ModDate'] = dt.datetime.today()
	return None
...
def ValidatePrediction(dbs, pmet, lmet):
	...
	with PdfPages(plotfname) as pdf:
		for l in range(1 + dbs.levels):
			...
			plt.plot(phyerr, logerr[:, l], label = ml.Metrics[pmet][1], color = ml.Metrics[pmet][3], marker = ml.Metrics[pmet][2], markersize = gv.marker_size, linestyle = 'None')
			fitlines[0, :] = cl.ComputeBestFitLine(np.column_stack((np.log10(phyerr), logerr[:, l])))
			fitlines[1, :] = cl.ComputeBestFitLine(np.column_stack((np.log10(macerr), logerr[:, l])))
			# If the reference line is: ln y = m1 (ln x) + c1
			# and the machine learnt line is: ln y = m2 (ln x) + c2,
			# then we need to scale the machine learning data by: ln x' -> (ln x) * m2/m1 + (c2 - c1) which is the same as: x' = x^(m2/m1) * exp(c2 - c1)
			# print("fitlines\n%s\nscale = %g" % (fitlines, fitlines[1, 0]/fitlines[0, 0]))
			# print("macerr\n%s" % (np.array_str(macerr)))
			plt.plot(np.power(macerr, fitlines[1, 0]/fitlines[0, 0]), logerr[:, l], label = "$\\epsilon_{\\rm predicted}$", color = 'blue', marker = 'o', markersize = gv.marker_size, linestyle = 'None')
			# Axes labels
			ax = plt.gca()
			ax.set_xlabel("$\\mathcal{N}_{0}$", fontsize = gv.axes_labels_fontsize)
			ax.set_xscale('log')
			ax.set_ylabel(("$\\widetilde{\\mathcal{N}}_{%d}$  $\\left(%s\\right)$" % (l, ml.Metrics[lmet][1].replace("$", ""))), fontsize = gv.axes_labels_fontsize)
			ax.set_yscale('log')
			ax.tick_params(axis = 'both', which = 'both', pad = gv.ticks_pad, direction = 'inout', length = gv.ticks_length, width = gv.ticks_width, labelsize = gv.ticks_fontsize)
			# Legend
			lgnd = plt.legend(numpoints = 1, loc = 4, shadow = True, fontsize = gv.legend_fontsize, markerscale = gv.legend_marker_scale)
			...
			bins = ComputeBinVariance(np.abs(phyerr), -np.log10(np.abs(logerr[:, l])))
			AddBinVariancePlot(bins, l, lmet, pmet, ml.Metrics[pmet][1], pdf)
			bins = ComputeBinVariance(np.power(np.abs(macerr), fitlines[1, 0]/fitlines[0, 0]), -np.log10(np.abs(logerr[:, l])))
			AddBinVariancePlot(bins, l, lmet, 0, "$\\epsilon(\\mathcal{E})$", pdf)
		#Set PDF attributes
		pdfInfo = pdf.infodict()
		pdfInfo['Title'] = ("Comparing predicted p to physical %s at levels %s, by studying fluctuations of output %s for %d channels." % (ml.Metrics[pmet][0], ", ".join(list(map(lambda str: "%d" % str, range(1, 1 + dbs.levels)))), ml.Metrics[lmet][0], dbs.channels))
		pdfInfo['Author'] = "Pavithran Iyer"
		pdfInfo['ModDate'] = dt.datetime.today()
	return None
...
def MCStatsPlot(dbses, lmet, pmet = -1):
	...
	with PdfPages(plotfname) as pdf:
		for s in range(1 + int(all([(dbses[d].importance == 2) for d in range(ndbses)]))):
			labels = []
			fig = plt.figure(figsize = gv.canvas_size)
			# Empty plots to indicate the databse (possibily with varying sampling methods) in the legend
			for d in range(ndbses):
				plt.plot([], [], color = 'k', linestyle = gv.line_styles[d % len(gv.line_styles)], label = ("%s sampling" % (", ".join([name for (name, num) in dbses[d].samplingOptions.items() if (num == dbses[d].importance)]))))
			for d in range(ndbses):
				for i in range(dbses[d].noiserates.shape[0]):
					if (sub.IsNumber(pmet) == 1):
						if (pmet == -1):
							label = "%s = %s" % (", ".join(qc.Channels[dbses[d].channel][2]), ",".join(list(map(lambda num: DisplayForm(num, 10.0), RealNoise(dbses[d].scales, phyerrs[d][i])))))
						else:
			...
			ax.set_ylabel(("$\\widetilde{\\mathcal{N}}_{%d}$  $\\left(%s\\right)$" % (dbses[0].levels, ml.Metrics[lmet][1].replace("$", ""))), fontsize = gv.axes_labels_fontsize)
		...
		#Set PDF attributes
		pdfInfo = pdf.infodict()
		pdfInfo['Title'] = ("Convergence of average logical %s with the number of syndrome samples for different %s at level %d." % (ml.Metrics[lmet][0], str(pmet), dbses[0].levels))
		pdfInfo['Author'] = "Pavithran Iyer"
		pdfInfo['ModDate'] = dt.datetime.today()
	return None
	...
	with PdfPages(plotfname) as pdf:
		for i in range(dbs.channels):
			...
			if (cont == 1):
				...
				nchans = nchans + 1
				bins = np.load(fn.SyndromeBins(dbs, dbs.available[i, :-1], dbs.available[i, -1], lmet))
				for l in range(1, dbs.levels):
					fig = plt.figure(figsize = (gv.canvas_size[0] * 2, gv.canvas_size[1] * 3))
					...
					plt.pcolor(meshY, meshX, meshZ, cmap = "binary", norm=LogNorm(vmin=np.min(meshZ[np.nonzero(meshZ)]), vmax=np.max(meshZ)))
					# Title
					# plt.title("p = %s, s = %d, l = %d" % (np.array_str(dbs.available[i, :-1]), int(dbs.available[i, -1]), l + 1), fontsize = gv.title_fontsize, y = 1.03)
					# Axes labels
					ax = plt.gca()
					ax.set_ylabel("$- $log${}_{10}\\mathsf{Pr}(s)$", fontsize = gv.axes_labels_fontsize + 144)
					ax.set_ylim([0,50])
					# yticks = nzrows[np.linspace(0, nzrows.shape[0] - 1, npoints, dtype = np.int)]
					# ax.set_yticks(yticks)
					# ax.set_yticklabels(["%d" % (tc) for tc in yticks])
					ax.set_xlabel("$- $log${}_{10}\\mathcal{N}(\\mathcal{E}^{\\thickspace s}_{%d})$" % (l + 1), fontsize = gv.axes_labels_fontsize + 144)
					# xticks = nzcols[np.linspace(0, nzcols.shape[0] - 1, npoints, dtype = np.int)]
					# ax.set_xticks(xticks)
					# ax.set_xticklabels(["%d" % (tc) for tc in xticks])
					ax.tick_params(axis = 'both', which = 'both', pad = gv.ticks_pad + 50, direction = 'inout', length = gv.ticks_length, width = gv.ticks_width, labelsize = gv.ticks_fontsize + 120)
					# Legend
					cbar = plt.colorbar(spacing = "proportional", drawedges = False)
					# cbar.ax.set_xlabel("$\\mathcal{T}_{%d}$" % (l + 1), fontsize = gv.colorbar_fontsize)
					cbar.ax.tick_params(labelsize = gv.legend_fontsize + 120, pad = gv.ticks_pad, length = gv.ticks_length, width = gv.ticks_width)
					cbar.ax.xaxis.labelpad = gv.ticks_pad
					# Save the plot
					pdf.savefig(fig)
					plt.close()
		#Set PDF attributes
		pdfInfo = pdf.infodict()
		pdfInfo['Title'] = ("Syndrome bins for %d channels and %d levels." % (nchans, dbs.levels))
		pdfInfo['Author'] = "Pavithran Iyer"
		pdfInfo['ModDate'] = dt.datetime.today()
	return None
...
def PlotBinVariance(dbses, lmet, pmet, nbins = 10):
	...
	if (sub.IsNumber(pmet) == 1):
		phyerr = np.squeeze(np.vstack(tuple([dbses[i].available[:, pmet] for i in range(len(dbses))])))
		pmetname = qc.Channels[dbses[0].channel][2][pmet]
	else:
		phyerr = np.hstack(tuple([np.load(fn.PhysicalErrorRates(dbses[i], pmet)) for i in range(len(dbses))]))
		pmetname = ml.Metrics[pmet][1]
	logerr = np.vstack(tuple([np.load(fn.LogicalErrorRates(dbses[i], lmet)) for i in range(len(dbses))]))
	...
	with PdfPages(plotfname) as pdf:
		for l in range(maxlevel + 1):
			# print("logerr[%d]" % (l))
			# print logerr[:, l]
			bins = ComputeBinVariance(np.abs(phyerr), -np.log10(np.abs(logerr[:, l])), nbins)
		...
		#Set PDF attributes
		pdfInfo = pdf.infodict()
		pdfInfo['Title'] = ("Binwise variance of %s with respect to %s." % (lmet, str(pmet)))
		pdfInfo['Author'] = "Pavithran Iyer"
		pdfInfo['ModDate'] = dt.datetime.today()
	return None


def ComputeBinVariance(xdata, ydata, nbins = 10, space = "log"):
	...
	if (space == "log"):
		window = np.power(base, np.linspace(np.log10(np.max(xdata))/np.log10(base), np.log10(np.min(xdata))/np.log10(base), nbins))[::-1]
	else:
	...
	print("\033[2mTotal: %d points and average variance = %g and maximum variance = %g.\033[0m" % (np.sum(bins[:, 2], dtype = int), np.mean(bins[:, 3]), np.max(bins[:, 3])))
	return bins
...
def ComputeNDimBinVariance(xdata, ydata, nbins = 3, space = "linear"):
	...
	for i in range(ndim):
		if (space == "log"):
			window[i, :] = np.power(base, np.linspace(np.log10(np.max(xdata[:, i]))/np.log10(base), np.log10(np.min(xdata[:, i]))/np.log10(base), nbins))[::-1]
		else:
	...
	for i in range(xdata.shape[0]):
		...
		binindex[i] = np.sum(np.multiply(address[i, :], np.power(nbins - 1, np.linspace(ndim - 1, 0, ndim))), dtype = np.int)
...
def AddBinVariancePlot(bins, level, lmet, pmet, pmetname, pdf = None, plotfname = "unknown"):
	# Plot the variance in each bin with respect to the bin along with producing a table of those values.
	...
	plt.title("level %d: Average scatter = %g, Maximum scatter = %g" % (level, np.mean(bins[:, 3]), np.max(bins[:, 3])), fontsize = gv.title_fontsize, y = 1.03)
	# print("widths\n%s" % (np.array_str(bins[:, 1] - bins[:, 0])))
	barplot = plt.bar(bins[:, 0], bins[:, 3], width = bins[:, 1] - bins[:, 0], bottom = 0, align = 'edge', color = color, linewidth = 0)
	# Axes
	ax = plt.gca()
	ax.set_xlabel("$-\\log10(\\mathcal{N}_{0}: %s)$" % (pmetname.replace("$","")), fontsize = gv.axes_labels_fontsize)
	# ax.set_xscale('log')
	ax.set_ylabel(("Variance in $\\log\\mathcal{N}_{%d}$  $\\left(%s\\right)$" % (level, ml.Metrics[lmet][1].replace("$", ""))), fontsize = gv.axes_labels_fontsize)
	ax.tick_params(axis = 'both', which = 'both', pad = gv.ticks_pad, direction = 'inout', length = gv.ticks_length, width = gv.ticks_width, labelsize = gv.ticks_fontsize)
	...
	for rect in barplot:
		...
		ax.text(rect.get_x() + width/float(2), 1.01 * height, "%g" % (height), ha = 'center', va = 'bottom', fontsize = gv.ticks_fontsize)
	if (pdf is None):
		# Save the plot
		pdfobj.savefig(fig)
		plt.close()
