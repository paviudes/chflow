...
def GatherLogErrData(submit):
	...
	for m in range(len(submit.metrics)):
		...
		runavg = np.zeros((submit.channels, 2, submit.stats.shape[0], 1 + int(submit.importance == 2)), dtype = np.longdouble)
		for i in range(submit.channels):
			...
			fname = fn.RunningAverageCh(submit, submit.available[i, :-1], submit.available[i, -1], submit.metrics[m])
...
def ComputeThreshold(dbs, lmet):
	...
	for i in range(dbs.channels):
		bestfit = ComputeBestFitLine(np.concatenate((np.arange(dbs.levels + 1)[:, np.newaxis], logErr[i, :, np.newaxis]), axis = 1))
...
def AddPhysicalRates(dbs):
	...
	for m in range(len(dbs.metrics)):
		...
		for i in range(dbs.channels):
			fname = fn.LogicalErrorRate(dbs, dbs.available[i, :-1], dbs.available[i, -1], dbs.metrics[m])
			logerr = np.load(fname)
			logerr[0] = phyerr[m, i]
			np.save(fname, logerr)
			print("\r\033[2m%d channels remaining.\033[0m" % (dbs.channels * len(dbs.metrics) - (m * dbs.channels + i + 1))),
	print("")
	return None