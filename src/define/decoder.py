...
def GetLeadingPaulis(lead_frac, qcode, chan_probs, option, max_weight = None):
	...
	if option == "full":
		...
		nrdata_weight = [int(np.ceil(lead_frac * n_errors_weight[w])) for w in range(max_weight + 1)]
		leading_paulis = np.zeros(sum(n_paulis_weight), dtype=np.int)
		
		for w in range(max_weight + 1):
			errors_wtw = qcode.group_by_weight[w]
			indices_picked = errors_wtw[np.argsort(iid_chan_probs[errors_wtw])[-nrdata_weight[w]:]]
			if (w == max_weight - 1):
				leading_paulis[nrdata_weight[w] : nrdata_weight[w + 1]] = indices_picked[:]
			else:
				leading_paulis[nrdata_weight[w] : ] = indices_picked[:]
