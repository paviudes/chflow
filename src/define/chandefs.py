...
def GetKraussForChannel(chType, *params):
	...
	rootunity = np.array([[np.exp(1j * np.pi * root), 0], [0, np.exp(-1j * np.pi * root)]], dtype = np.complex128)
	...
	if chType == "id":
		...
		krauss = np.zeros((1, 2, 2), dtype = np.complex128)
		krauss[0, :, :] = linalg.expm(-1j * np.pi * (np.random.normal(loc = params[0], scale = 1.0)) * gv.Pauli[3, :, :])
		...
		krauss = np.zeros((1, 2, 2), dtype = np.complex128)
		krauss[0, :, :] = linalg.expm(-1j * np.pi * (np.random.normal(loc = params[0], scale = 1.0)) * gv.Pauli[1, :, :])
		...
		# Random second order perturbations to a rotation about the Z-axis
		krauss = np.zeros((1, 2, 2), dtype = np.complex128)
		krauss[0, :, :] = linalg.expm(-1j * np.pi * (np.random.normal(loc = params[0], scale = 1.0)) * gv.Pauli[2, :, :])
		...
		phi = params[1]
		theta = params[2]
		axis = np.array([np.sin(theta) * np.cos(phi), np.sin(theta) * np.sin(phi), np.cos(theta)], dtype = np.longdouble)
		...
		dephase = params[1]
		krauss = np.zeros((3, 2, 2), dtype = np.complex128)
		krauss[0, :, :] = np.sqrt(1 - dephase) * linalg.expm(-1j * np.pi * params[0] * gv.Pauli[3, :, :])
		krauss[1, :, :] = np.sqrt(dephase) * np.dot(linalg.expm(-1j * np.pi * params[0] * gv.Pauli[3, :, :]), gv.Pauli[3, :, :])
		...
		krauss = np.zeros((2, 2, 2), dtype = np.complex128)
		krauss[0, :, :] = np.sqrt(1 - params[0]) * np.eye(2)
		krauss[1, :, :] = np.sqrt(params[0]) * linalg.expm(-1j * np.pi * params[1] * params[2])
		...
		krauss[0, :, :] = np.sqrt(1 - params[0] - np.power(params[0], 1/np.longdouble(params[1]))) * np.eye(2)
		krauss[1, :, :] = np.sqrt(params[0]/3) * gv.Pauli[1, :, :]
		krauss[2, :, :] = np.sqrt(params[0]/3) * gv.Pauli[2, :, :]
		krauss[3, :, :] = np.sqrt(params[0]/3) * gv.Pauli[3, :, :]
		krauss[4, :, :] = np.power(params[0], 1/np.longdouble(2 * params[1])) * hadamard
		...
		if (len(params) < 2):
			krauss = rchan.RandomCPTP(params[0], 0)
		else:
