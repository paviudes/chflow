...
def GeneralizedDamping(relax, dephase):
    ...
    krauss[0, :, :] = np.sqrt(1 - dephase) * np.array(
        [[1, 0], [0, np.sqrt(1 - relax)]], dtype=np.complex128
    )
    krauss[1, :, :] = np.sqrt(1 - dephase) * np.array(
        [[0, np.sqrt(relax)], [0, 0]], dtype=np.complex128
    )
    krauss[2, :, :] = np.sqrt(dephase) * np.array(
        [[1, 0], [0, (-1) * np.sqrt(1 - relax)]], dtype=np.complex128
    )
    krauss[3, :, :] = np.sqrt(dephase) * np.array(
        [[0, np.sqrt(relax)], [0, 0]], dtype=np.complex128
    )
    return krauss
...
def GDPerfPlots(submit, logmet, xcol=0, ycol=1):
    ...
    (meshX, meshY) = np.meshgrid(
        np.linspace(xaxis.min(), xaxis.max(), max(100, xaxis.shape[0])),
        np.linspace(xaxis.min(), xaxis.max(), max(100, yaxis.shape[0])),
    )
    ...
    with PdfPages(plotfname) as pdf:
        ...
        for l in range(submit.levels):
            ...
            meshZ = np.abs(
                griddata(
                    (xaxis, yaxis), logErr[:, l + 1], (meshX, meshY), method="cubic"
                )
            )
            ...
            ax = plt.gca()
            ax.set_xlabel(
                qc.Channels[submit.channel][2][xcol], fontsize=gv.axes_labels_fontsize
            )
            ax.set_xscale("log")
            ax.set_ylabel(
                qc.Channels[submit.channel][2][ycol], fontsize=gv.axes_labels_fontsize
            )
            ax.set_yscale("log")
            ax.tick_params(
                axis="both",
        ...
        pdfInfo["Title"] = "%s at levels %s, for %d Generalized damping channels." % (
            ml.Metrics[logmet][0],
...
def GenTicksInfo(axis, base, npoints):
    ...
    ticklabels = [
        "$%g^{-%g}$" % (1 / base, expo)
        for expo in np.linspace(axis[0], axis[-1], npoints)
    ]
...
def GDPerfTimeScales(submit, logmet, colx=0, coly=1):
    ...
    if submit.scale == 1:
        (meshX, meshY) = np.meshgrid(
            np.linspace(
                submit.noiserates[:, colx].min(),
                submit.noiserates[:, colx].max(),
                max(100, submit.noiserates.shape[0]),
            ),
            np.linspace(
                submit.noiserates[:, coly].min(),
                submit.noiserates[:, coly].max(),
                max(100, submit.noiserates.shape[0]),
            ),
        )
    else:
        (meshX, meshY) = np.meshgrid(
            np.linspace(
                np.power(submit.scale, submit.noiserates[:, colx]).min(),
                np.power(submit.scale, submit.noiserates[:, colx]).max(),
                max(100, submit.noiserates.shape[0]),
            ),
            np.linspace(
                np.power(submit.scale, submit.noiserates[:, coly]).min(),
                np.power(submit.scale, submit.noiserates[:, coly]).max(),
                max(100, submit.noiserates.shape[0]),
            ),
        )
    ...
    with PdfPages(plotfname) as pdf:
        ...
        for l in range(1 + submit.levels):
            ...
            if submit.scale == 1:
                meshZ = griddata(
                    (submit.noiserates[:, colx], submit.noiserates[:, coly]),
                    logErr[:, l],
                    (meshX, meshY),
                    method="nearest",
                )
                ...
                meshZ = griddata(
                    (
                        np.power(submit.scale, submit.noiserates[:, colx]),
                        np.power(submit.scale, submit.noiserates[:, coly]),
                    ),
                    logErr[:, l],
                    (meshX, meshY),
                    method="linear",
            ...
            if submit.scale == 1:
                plt.scatter(
                    submit.noiserates[:, colx],
                    submit.noiserates[:, coly],
                    marker="o",
                    color="k",
                )
            else:
...
if __name__ == "__main__":
    ...
    print(
        "The Krauss operators for the Generalized Damping channel with relax = %g, dephase = %g are the following.\n%s"
        % (relax, dephase, np.array_str(gdkrauss))
    )
    gdprocess = crep.ConvertRepresentations(gdkrauss, "krauss", "process")
    print(
        "The process matrix for the Photon Loss channel with relax = %g, dephase = %g is the following.\n%s"
        % (relax, dephase, np.array_str(gdprocess))
    )
    print("Physical metrics for the channel")
    metrics = ["fidelity", "frb"]
    metvals = ml.ComputeNorms(
        crep.ConvertRepresentations(gdprocess, "process", "choi"), metrics
    )
