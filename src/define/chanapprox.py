...
def MatrixPositivityConstraints(proba, inchan):
	...
	pauli = np.array([[np.power(1 - pI - proba[0] + proba[1] + proba[2], 2.0), 0.0, 0.0],
					  [0.0, np.power(1 - pI + proba[0] - proba[1] + proba[2], 2.0), 0.0],
					  [0.0, 0.0, np.power(1 - pI + proba[0] + proba[1] - proba[2], 2.0)]], dtype = np.float)
...
def DiamondDistanceFromPauli(proba, refchan):
	...
	pI = 1 - np.sum(proba, dtype = np.float)
	pchoi = np.array([[pI + proba[2], 0, 0, pI - proba[2]],
					  [0, proba[0] + proba[1], proba[0] - proba[1], 0],
					  [0, proba[0] - proba[1], proba[0] + proba[1], 0],
					  [pI - proba[2], 0, 0, pI + proba[2]]], dtype = np.float) * 0.5
	diff = np.real(((refchan - pchoi) + HermitianConjugate(refchan - pchoi))/np.float(2))
...
def HonestPauliApproximation(channel, rep = "process"):
	...
	if (not (rep == "process")):
		process = np.real(crep.ConvertRepresentations(channel, rep, "process")).astype(float)
	else:
	...
	nonunital = np.linalg.norm(translation, ord = 'fro') + 2 * np.linalg.norm(np.dot(np.transpose(np.eye(3, dtype = np.float) - rotation), translation), ord = 'fro')
	blocherr = np.dot(np.transpose(np.eye(3, dtype = np.float) - rotation), (np.eye(3, dtype = np.float) - rotation)) + nonunital * np.eye(3, dtype = np.float)
	...
	cons.append({'type':'ineq', 'fun': (lambda pp: MatrixPositivityConstraints(pp, blocherr))})
	initGuess = np.array([0.25, 0.25, 0.25], dtype = float)
	start = time.time()
	print("\033[2m"),
	result = opt.minimize(objective, initGuess, jac = None, constraints = cons, bounds = [(0,1), (0,1), (0,1)], method = 'SLSQP', options = {'disp':True, 'maxiter':5000})
	print("\033[0m"),
	...
	if (result.success == True):
		print("\t\033[2mOptimization completed successfully in %d seconds.\033[0m" % (runtime))
	else:
		print("\t\033[2mOptimization terminated because of\n%s\nin %d seconds.\033[0m" % (result.message, runtime))
	result.clear()
	return (proba, proxim)
