...
def NRWeightsFile(dbs, noise, sample):
    # Name of the file containing the physical channel
    noisedes = "_".join(list(map(lambda p: ("%g" % p), noise)))
    fname = "%s/channels/nrw_%s_%s_s%d.npy" % (dbs.outdir, dbs.channel, noisedes, sample)
    return fname
...
def CompareSubsPlot(dbs, other_dbs_names):
    # File containing the plot of comparing the logical error rates from two (or more) submission records.
    fname = "%s/results/compare_%s.pdf" % (dbs.outdir, "_".join(other_dbs_names))
    return fname
