...
def r(alpha):
	# coefficients for simplifying the expression for the Krauss operators.
	return ((np.power(epsilon(alpha), 4.0) - 1) * np.power(np.abs(alpha), 2.0)/np.power(epsilon(alpha), 2.0))
...
def p(alpha):
	# coefficients for simplifying the expression for the Krauss operators.
	return (np.power((np.power(epsilon(alpha), 2.0) + 1), 2.0) * np.power(np.abs(alpha), 2.0)/(2 * np.power(epsilon(alpha), 2.0)))
...
def Fp(gamma, alpha):
	# coefficients for simplifying the expression for the Krauss operators.
	return np.sqrt(np.power(M(gamma, alpha) + P(gamma, alpha), 2.0) + np.power(R(gamma, alpha), 2.0))
...
def F1(gamma, alpha):
	# coefficients for simplifying the expression for the Krauss operators.
	return np.sqrt(Fm(gamma, alpha)/(M(gamma, alpha) - P(gamma, alpha) + Fm(gamma, alpha)))
...
def F3(gamma, alpha):
	# coefficients for simplifying the expression for the Krauss operators.
	return np.sqrt(Fp(gamma, alpha) * (M(gamma, alpha) + P(gamma, alpha) + Fp(gamma, alpha)))/np.longdouble(R(gamma, alpha))

def F4(gamma, alpha):
	# coefficients for simplifying the expression for the Krauss operators.
	return np.sqrt(Fp(gamma, alpha)/(M(gamma, alpha) + P(gamma, alpha) + Fp(gamma, alpha)))
...
def K1(gamma, alpha):
	...
	krop[0, 1] = 1j * (- M(gamma, alpha) + P(gamma, alpha) - R(gamma, alpha) + Fm(gamma, alpha))/(2 * R(gamma, alpha) * F1(gamma, alpha))
	krop[1, 0] = 1j * (- M(gamma, alpha) + P(gamma, alpha) + R(gamma, alpha) + Fm(gamma, alpha))/(2 * R(gamma, alpha) * F1(gamma, alpha))
	return krop

def K2(gamma, alpha):
	# return the expression for the second Krauss operator
	krop = np.zeros((2, 2), dtype = np.complex128)
	krop[0, 1] = - 1j * (M(gamma, alpha) - P(gamma, alpha) + R(gamma, alpha) + Fm(gamma, alpha))/(2 * R(gamma, alpha) * F2(gamma, alpha))
	krop[1, 0] = 1j * (- M(gamma, alpha) + P(gamma, alpha) + R(gamma, alpha) - Fm(gamma, alpha))/(2 * R(gamma, alpha) * F2(gamma, alpha))
	return krop

def K3(gamma, alpha):
	# return the expression for the third Krauss operator
	krop = np.zeros((2, 2), dtype = np.complex128)
	krop[0, 0] = (- M(gamma, alpha) - P(gamma, alpha) + R(gamma, alpha) - Fp(gamma, alpha))/(2 * R(gamma, alpha) * F3(gamma, alpha))
	krop[1, 1] = (M(gamma, alpha) + P(gamma, alpha) + R(gamma, alpha) + Fp(gamma, alpha))/(2 * R(gamma, alpha) * F3(gamma, alpha))
	return krop

def K4(gamma, alpha):
	# return the expression for the fourth Krauss operator
	krop = np.zeros((2, 2), dtype = np.complex128)
	krop[0, 0] = (- M(gamma, alpha) - P(gamma, alpha) + R(gamma, alpha) + Fp(gamma, alpha))/(2 * R(gamma, alpha) * F4(gamma, alpha))
	krop[1, 1] = (M(gamma, alpha) + P(gamma, alpha) + R(gamma, alpha) - Fp(gamma, alpha))/(2 * R(gamma, alpha) * F4(gamma, alpha))
	return krop
...
def oldK1(gamma, alpha):
	...
	krop = np.zeros((2, 2), dtype = np.complex128)
	krop[[0, 1], [0, 1]] = np.sqrt(np.power((1 - gamma), np.power(alpha, 2.0)) + np.power((1 - gamma), (-1) * np.power(alpha, 2.0)))/np.sqrt(2.0)
	return krop
...
def oldK2(gamma, alpha):
	...
	krop = np.zeros((2, 2), dtype = np.complex128)
	krop[0, 1] = np.sqrt((np.power((1 - gamma), (-1) * np.power(alpha, 2.0)) - np.power((1 - gamma), np.power(alpha, 2.0)))/(np.longdouble(2.0) * np.tanh(np.power(alpha, 2.0))))
	...
	with PdfPages(plotfname) as pdf:
		for a in range(submit.noiserange[1].shape[0]):
			...
			for l in range(submit.levels):
				...
				yaxis = logErrorRates[np.arange(submit.noiserange[0].shape[0]) * submit.noiserange[1].shape[0] + a, l]
				plt.plot(xaxis, yaxis, label = ("N = %d, d = %d" % (nqubits, dist)), color = ml.Metrics[ml.Metrics.keys()[l]][3], marker = ml.Metrics[ml.Metrics.keys()[l]][2], markersize = gv.marker_size, linestyle = '--', linewidth = gv.line_width)
			# Title
			plt.title("$\\alpha = %g$" % (submit.noiserange[1][a]), fontsize = gv.title_fontsize, y = 1.03)
			# Legend
		...
		pdfInfo['Title'] = ("%s at levels %s, with gamma for %d channels." % (ml.Metrics[logmet][0], ", ".join(map(str, range(1, 1 + submit.levels))), submit.channels))
...
def PLPerfPlots(submit, logmet):
	...
	with PdfPages(plotfname) as pdf:
		...
		for l in range(submit.levels):
			...
			for a in range(submit.noiserange[1].shape[0]):	
				yaxis = logErrorRates[np.arange(submit.noiserange[0].shape[0]) * submit.noiserange[1].shape[0] + a, l]
				plt.plot(xaxis, yaxis, label = "$\\alpha = %g$" % (submit.noiserange[1][a]), color = ml.Metrics[ml.Metrics.keys()[a]][3], marker = ml.Metrics[ml.Metrics.keys()[a]][2], markersize = gv.marker_size, linestyle = '--', linewidth = gv.line_width)
			# Title
...
def PLPerfPlots2D(submit, logmet):
	...
	(meshX, meshY) = np.meshgrid(np.linspace(submit.noiserates[:, 0].min(), submit.noiserates[:, 0].max(), max(100, submit.noiserates.shape[0])), np.linspace(submit.noiserates[:, 1].min(), submit.noiserates[:, 1].max(), max(100, submit.noiserates.shape[0])))
	...
	with PdfPages(plotfname) as pdf:
		...
		for l in range(submit.levels):
			...
			dist = dist * submit.eccs[l].D
			fig = plt.figure(figsize = gv.canvas_size)
			meshZ = griddata((submit.noiserates[:, 0], submit.noiserates[:, 1]), logErr[:, l + 1], (meshX, meshY), method = "cubic")
			...
			plt.scatter(submit.noiserates[:, 0], submit.noiserates[:, 1], marker = 'o', color = 'k')
			# Title
			plt.title("N = %d, d = %d" % (nqubits, dist), fontsize = gv.title_fontsize, y = 1.03)
			# Axes labels
			ax = plt.gca()
		...
		#Set PDF attributes
		pdfInfo = pdf.infodict()
		pdfInfo['Title'] = ("%s at levels %s, for %d Photon loss channels." % (ml.Metrics[logmet][0], ", ".join(map(str, range(1, 1 + submit.levels))), submit.channels))
		pdfInfo['Author'] = "Pavithran Iyer"
		pdfInfo['ModDate'] = dt.datetime.today()
	return None
