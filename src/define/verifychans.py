...
def IsUnital(channel, rep="process"):
    ...
    if np.allclose(
        crep.ConvertRepresentations(channel, rep, "process")[0, :],
        np.array([1, 0, 0, 0], dtype=np.longdouble),
    ):
        return 1
    return 0
...
def IsQuantumChannel(channel, rep="process"):
    ...
    if np.prod(conditions, dtype=np.int8) == 0:
        print(
            "! E = \n%s\nis not a valid quantum channel because"
            % (np.array_str(channel, max_line_width=100))
        )
        if conditions[0] == 0:
