...
from printfuns cimport PrintDoubleArray1D, PrintComplexArray2D, PrintIntArray1D
...
void InitQECC(qecc_t *qecc, int nclifford){
	...
	for (i = 0; i < qecc->nlogs; i ++){
		...
		for (g = 0; g < qecc->nstabs; g ++){
			(qecc->action)[i][g] = malloc(qecc->N * sizeof(int));
			for (r = 0; r < qecc->N; r ++){
				(qecc->action)[i][g][r] = 0;
			}
		}
	...
	qecc->phases = malloc(qecc->nlogs * sizeof(complex128_t *));
	for (i = 0; i < qecc->nlogs; i ++){
		(qecc->phases)[i] = malloc(qecc->nstabs * sizeof(complex128_t));
		for (g = 0; g < qecc->nstabs; g ++){
			(qecc->phases)[i][g] = 0 + 0 * I;
		}
	}

	qecc->algebra = malloc(2 * sizeof(int **));
	for (i = 0; i < 2; i ++){
		(qecc->algebra)[i] = malloc(nclifford * sizeof(int *));
		for (r = 0; r < nclifford; r ++){
			(qecc->algebra)[i][r] = malloc(qecc->nlogs * sizeof(int));
			for (c = 0; c < qecc->nlogs; c ++){
				(qecc->algebra)[i][r][c] = 0;
			}
		}
	}
}
...
void FreeQECC(qecc_t *qecc, int nclifford){
	// Free the memory assigned to a quantum error correcting code.
	int i, s, r, c, g;
	for (s = 0; s < qecc->nstabs; s ++){
...
void SingleShotErrorCorrection(int level, int isPauli, int frame, qecc_t *qecc, simul_t *sim, constants_t *consts){
	// Compute the effective logical channel, when error correction is applied over a set of input physical channels.
	// printf("Constructing the full process matrix\n");
	GetFullProcessMatrix(qecc, sim, isPauli, level);
	// Compute the probabilities of all the syndromes.
	ComputeSyndromeDistribution(qecc, sim, isPauli, consts->atol);
	// Maximum Likelihood Decoding (MLD) -- For every syndrome, compute the probabilities of the logical classes and pick the one that is most likely.
...
void ChoiToProcess(long double **process, complex128_t **choi, complex128_t pauli[4][2][2]){
...
cdef int ComputeEffectiveStates(qecc_t *qecc, simul_t *sim, constants_t *consts, int isPauli) nogil:
	...
	for s in range(qecc[0].nstabs):
		...
		if (sim[0].syndprobs[s] > consts[0].atol):
			...
			# with gil:
			# 	print("rho_%d, P(s) = %g, isPauli = %d" % (s, sim[0].syndprobs[s], isPauli))
			# 	for r in range(qecc[0].nlogs):
			# 		for c in range(qecc[0].nlogs):
			# 			print("%g + i %g   " % (creall(sim[0].effective[s][r][c]), cimagl(sim[0].effective[s][r][c]))),
			# 		print("")
			# Normalization and rounding
			for r in range(qecc[0].nlogs):
				for c in range(1 + r):
					if (fabsl(creall(sim[0].effective[s][r][c])) < (consts[0].atol)):
						sim[0].effective[s][r][c] = 0 + 1j * cimagl(sim[0].effective[s][r][c])
					if (fabsl(cimagl(sim[0].effective[s][r][c])) < (consts[0].atol)):
			...
			# 			print("%g + i %g   " % (creall(sim[0].effective[s][r][c]), cimagl(sim[0].effective[s][r][c]))),
			# 		print("")
			# PrintComplexArray2D(sim[0].effective[s], "effective", 4, 4)
	return 0

####################
# Converted to C.

void ComputeEffectiveStates(qecc_t *qecc, simul_t *sim, constants_t *consts, int isPauli){
	// Compute the Choi matrix of the effective logical channel, after applying noise + error correction.
	// The effective Choi matrix (state) is given by: L Ts PI_s E(r) PI_s T_s L which can be written in its unencoded form by expanding in the Pauli basis.
	// un-encoded state = sum_(a,b) Tr( L Ts PI_s E(r) PI_s T_s L . (PI_0 Pa PI_0 \o Pb)).
	// CHOI[a,b] = sum_(i: Pi is in the logical class of Pb) sum_(j: Pj is in the logical class of [L Pa L]) CHI[i,j] * (-1)^(P_j) * (-1)**(if {L, Pb} == 0).
		...
		if ((sim->syndprobs)[s] > consts->atol){
			if (isPauli == 0){
					...
					for (b = 0; b < qecc->nlogs; b ++){
						...
						for (i = 0; i < qecc->nstabs; i ++){
							for (j = 0; j < qecc->nstabs; j ++){
								coeff = coeff + (sim->process)[b][(qecc->algebra)[0][(sim->corrections)[s]][a]][i][j] * (qecc->projector)[s][j];
							}
						}
						for (r = 0; r < qecc->nlogs; r ++){
							for (c = 0; c < 1 + r; c ++){
								(sim->effective)[s][r][c] = (sim->effective)[s][r][c] + coeff * powl(-1, (<int>(b == 2))) * (qecc->algebra)[1][(sim->corrections)[s]][a] * consts->pauli[a][r/2][c/2] * consts->pauli[b][r%2][c%2];
							}
						}
					}
				}
				...
				for (a = 0; a < qecc->nlogs; a ++){
					coeff = 0.0 + 0.0 * I;
					for (i = 0; i < qecc->nstabs; i ++){
						coeff = coeff + (sim->process)[a][(qecc->algebra)[0][(sim->corrections)[s]][a]][i][i] * (qecc->projector)[s][i]
					}
					for (r = 0; r < qecc->nlogs; r ++){
						for (c = 0; c < 1 + r; c ++){
							(sim->effective)[s][r][c] = (sim->effective)[s][r][c] + coeff * powl(-1, (<int>(a == 2))) * (qecc->algebra)[1][(sim->corrections)[s]][a] * consts->pauli[a][r/2][c/2] * consts->pauli[a][r%2][c%2]
						}
					}
				}
			}
			...
			for (r = 0; r < qecc->nlogs; r ++){
				for (c = 0; c < 1 + r; c ++){
					if (fabsl(creall((sim->effective)[s][r][c])) < (consts->atol)){
						(sim->effective)[s][r][c] = 0 + I * cimagl((sim->effective)[s][r][c]);
					}
					if (fabsl(cimagl((sim->effective)[s][r][c])) < (consts->atol)){
						(sim->effective)[s][r][c] = creall((sim->effective)[s][r][c]) + I * 0.0;
					}
				}
			}
			for (r = 0; r < qecc->nlogs; r ++){
				for (c = r + 1; c < qecc->nlogs; c ++){
					(sim->effective)[s][r][c] = conjl((sim->effective)[s][c][r]);
				}
			}

			for (r = 0; r < qecc->nlogs; r ++){
				for (c = 0; c < qecc->nlogs; c ++){
					(sim->effective)[s][r][c] = (sim->effective)[s][r][c]/((sim->syndprobs)[s] * (4 * <long double>(qecc->nstabs)));
				}
			}
		}
	}
		...
		if (sim[0].syndprobs[s] > consts[0].atol):
			...
			for l in range(currentframe):
				...
				if (prob > maxprob):
					...
					maxprob = prob
	# PrintIntArray1D(sim[0].corrections, "sim[0].corrections", qecc[0].nstabs)
	return 0

##################
# Converted to C.

void MLDecoder(qecc_t *qecc, simul_t *sim, constants_t *consts, int currentframe, int isPauli){
	// Perform maximum likelihood decoding.
	// Compute the probabilities of the logical classes, considitioned on a particular syndrome.
	// The ML Decoder picks the logical error which belongs to the class that has the maximum probability.
	// The probability of a logical class is P(L|s) = Tr( L r L . Ts PI_s E(r) PI_s Ts )/P(s) which can be simplified to
			...
			for (l = 0; l < currentframe; l ++){
				...
				if (isPauli == 0){
					for (u = 0 ; u < qecc->nlogs; u ++){
						...
						for (i = 0; i < qecc->nstabs; i ++){
							for (j = 0; j < qecc->nstabs; j ++){
								contrib = contrib + (sim->process)[u][(qecc->algebra)[0][l][u]][i][j] * (qecc->projector)[s][j];
							}
						}
						prob = prob + (qecc->algebra)[1][l][u] * contrib;
					}
				}
				else{
					for (u = 0 ; u < qecc->nlogs; u ++){
...
cdef int ComputeSyndromeDistribution(qecc_t *qecc, simul_t *sim, int isPauli, long double atol) nogil:
	...
	# PrintDoubleArray1D(sim[0].syndprobs, "syndrome probabilities", qecc[0].nstabs)
	return 0


##################
void ComputeSyndromeDistribution(qecc_t *qecc, simul_t *sim, int isPauli, long double atol){
	// Compute the probability of all the syndromes in the qecc code, for the given error channel and input state. Sample a syndrome from the resulting probability distribution of the syndromes.
	// Probability of a syndrome s, denoted by P(s) is given by the following expression.
	// P(s) = 1/2^(n-k) * sum_(i,j: P_i and P_j are stabilizers) CHI[i,j] * (-1)^sign(P_j).
	int s;
	for (s = 0; s < qecc->nstabs; s ++){
		...
		if (isPauli == 0):
			for (i = 0; i < qecc->nstabs; i ++){
				for (j = 0; j < qecc->nstabs; j ++){
					(sim->syndprobs)[s] = (sim->syndprobs)[s] + (sim->process)[0][0][i][j] * (qecc->projector)[s][j];
				}
			}
		else
			for (i = 0; i < qecc->nstabs; i ++){
		...
		(sim->syndprobs)[s] = (sim->syndprobs)[s]/((long double)(qecc->nstabs));
		if (s == 0){
...
void GetFullProcessMatrix(qecc_t *qcode, simul_t *sim, int isPauli, int level){
	// For each pair of logical operators, we only need the entries of the Chi matrix that correspond to Pauli operators from different logical classes.
	// We construct the sections of the Chi matrices that correspond to rows and columns in the logical classes.
	unsigned int i, j, k, q;
	long double contribution = 1.0;
	...
	if (isPauli == 0){
		for (i = 0; i < qcode->nlogs; i ++){
			for (j = 0; j < qcode->nlogs; j ++){
				for (k = 0; k < qcode->nstabs; k ++){
					for (l = 0; l < qcode->nstabs; l ++){
						contribution = 1.0;
						for (q = 0; q < qcode->N; q ++){
							contribution = contribution * (sim->virtual)[q][(qcode->action)[i][k][q]][(qcode->action)[j][l][q]];
						}
						(sim->process)[i][j][k][l] = creall((qcode->phases)[i][k] * (qcode->phases)[j][l]) * contribution;
					}
				}
			}
		}
		...
		for (i = 0; i < qcode->nlogs; i ++){
			for (j = 0; j < qcode->nlogs; j ++){
				contribution = 1.0;
				for (q = 0; q < qcode->N; q ++){
					contribution = contribution * (sim->virtual)[q][(qcode->action)[i][j][q]][(qcode->action)[i][j][q]];
				}
				(sim->process)[i][i][j][j] = <long double>((qcode->phases)[i][j] * (qcode->phases)[i][j]) * contribution;
			}
		}
	}
}
