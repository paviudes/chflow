	...
	for (s = 0; s < qecc->nstabs; s ++){
		if ((sim->syndprobs)[s] > atol){
			...
			for (l = 0; l < currentframe; l ++){
				...
				printf("s = %d, l = %d, prob = %g\n", s, l, prob/(qecc->nlogs * qecc->nstabs * (sim->syndprobs)[s]));
				if (prob > maxprob){
					(sim->corrections)[s] = l;
					maxprob = prob;
				}
						...
						for (a = 0; a < qecc->nlogs; a ++)
							for (b = 0; b < qecc->nlogs; b ++)
								if ((consts->algebra)[0][(sim->corrections)[s]][a] == b)
									(sim->effective)[s][i][j] += pow(-1, ((int)(b == 2))) * (consts->algebra)[1][(sim->corrections)[s]][a] * DiagGDotIntV((sim->process)[b][b], (qecc->projector)[s], qecc->nstabs, qecc->nstabs, qecc->nstabs) * (consts->pauli)[a][i/2][j/2] * (consts->pauli)[b][i%2][j%2];
			// Normalization
			for (i = 0; i < qecc->nlogs; i ++)
				for (j = 0; j < qecc->nlogs; j ++)
					(sim->effective)[s][i][j] /= (4 * (double)((sim->syndprobs)[s] * qecc->nstabs));
			// printf("s = %d, P(s) = %g\n", s, (sim->syndprobs)[s]);
